...
const merge = (intervals) => {
	...
	for (let i = 1; i < intervals.length; i++) {
		...
		if (start <= prevEnd) {
			result.at(-1)[1] = Math.max(prevEnd, end);
		} else {
...
class TimeMap {
	...
	set(key, value, timestamp) {
		const keyVals = this.map[key] ? this.map[key] : [];
		keyVals.push([value, timestamp]);
		this.map[key] = keyVals;
	}

	get(key, timestamp) {
		const valueTimestamp = this.map[key] ? this.map[key] : [];

		let left = 0;
		let right = valueTimestamp.length - 1;
		let result;

		while (left <= right) {
			let mid = Math.floor((left + right) / 2);

			if (valueTimestamp[mid][1] <= timestamp) {
				result = valueTimestamp[mid][1];
				left = mid + 1;
			} else {
				right = mid - 1;
			}
